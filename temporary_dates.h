...
class temporary_dates {
private:
	static temporary_dates* singeltone_;
	temporary_dates() = default;
	~temporary_dates() = default;
	temporary_dates(temporary_dates& other) = delete;
	temporary_dates(temporary_dates&& other) = delete;
	void operator=(const temporary_dates&) = delete;

	std::string points_[16] = { "msk_AP","msk_TS","msk_WH","mzh_TS",
...
"vlg_AP","vlg_TS","vlg_WH","kam_TS","kam_WH" };
	...
	std::tuple<int, int, double> tableCost_[21] = {
	{500, 300, 100},
	{200, 100, 500},
	{100, 60, 50},
	{0, 0, 0},
...
public:
	static temporary_dates* GetInstance() {
		if (singeltone_ == nullptr)
			singeltone_ = new temporary_dates();
		return singeltone_;
	}

	std::string GetPoints(int index) { return points_[index]; }

	std::map<std::string, int> GetTowns() { return towns_; }

	std::map<std::string, int> GetTable() { return table_; }

	std::tuple<int, int, double> GetTableCost(int index) { return tableCost_[index]; }

	int GetMatrixDist(int row, int column) { return matrixDist_[row][column]; }
};
